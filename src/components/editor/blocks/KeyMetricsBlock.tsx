...
export const KeyMetricsBlock: React.FC<KeyMetricsBlockProps> = () => {
    const metrics = [
        ...
        { label: 'CPA', value: '12.50€', icon: <Target size={28} />, color: 'text-primary', bg: 'from-primary-50 to-cyan-50 dark:from-primary-900/20 dark:to-primary-900/20' },
        ...
        <div className="p-6 bg-white/80 dark:bg-black/80 backdrop-blur-sm rounded-xl border border-neutral-200/50 dark:border-white/10 shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-4">
                {metrics.map((metric) => (
                    <div
                        key={metric.label}
                        className={`p-4 bg-gradient-to-br ${metric.bg} rounded-lg border border-neutral-200/50 dark:border-white/10`}
                    >
                        <div className={`mb-2 ${metric.color}`}>{metric.icon}</div>
                        <p className="text-sm text-neutral-600 dark:text-neutral-400">{metric.label}</p>
                        <p className="text-2xl font-bold text-neutral-900 dark:text-neutral-200">{metric.value}</p>
                    </div>
                ))}
            </div>
        </div>
