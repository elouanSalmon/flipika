...
/**
 * Cloud Function that runs every 15 minutes to generate scheduled reports
...
function calculateNextRunTime(config: any): Date {
    ...
    switch (config.frequency) {
        ...
        case "weekly":
            ...
            const weeklyTargetHourUTC = (config.hour || 0) - parisOffsetHours;
            next.setUTCHours(weeklyTargetHourUTC < 0 ? weeklyTargetHourUTC + 24 : weeklyTargetHourUTC, 0, 0, 0);

            if (weeklyTargetHourUTC < 0 && daysUntilTarget === 0) {
                next.setUTCDate(next.getUTCDate() + 1);
            }
        ...
        case "monthly":
            ...
            const monthlyTargetHourUTC = (config.hour || 0) - parisOffsetHours;
            next.setUTCHours(monthlyTargetHourUTC < 0 ? monthlyTargetHourUTC + 24 : monthlyTargetHourUTC, 0, 0, 0);
            ...
            while (next.getUTCDate() !== targetDayOfMonth && next.getUTCDate() < targetDayOfMonth) {
                next.setUTCDate(0); // Go to last day of previous month
            }
            break;
